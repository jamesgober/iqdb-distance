# iqdb-distance -- Engineering Directives
...
iqdb-distance is the innermost loop of the database: every search computes thousands of distances, so this crate is optimized aggressively while keeping a readable scalar reference. It provides every metric vector search needs, with SIMD implementations that are property-tested to match the scalar ground truth within floating-point tolerance.
...
- SIMD output must equal the scalar reference within tolerance (property-tested over random vectors).
- The scalar path stays readable enough to serve as the auditable definition of each metric.
...
- `iqdb-types`: the `DistanceMetric` enum and vector types
